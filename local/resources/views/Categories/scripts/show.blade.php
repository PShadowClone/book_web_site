<script>
    const BASE_URL = '{{route('category.show.all')}}';
    var url = BASE_URL;
    const DELETE_URL = '{{route('category.remove')}}';
</script>
...
<script>
    ...
    function drawDatatable(url) {
        ...
        var category_table = $('#category').on('error.dt', function (e, settings, techNote, message) {
            ...
            "columns": [
                ...
                {"data": "name"},
                {
                    "data": "id", mRender: function (data, type, row, full) {
                        return '<a class="btn btn-warning" href="{{route('category.edit')}}/' + row.id + '" > <i class="fa fa-edit "></i>  تعديل</a>' +
